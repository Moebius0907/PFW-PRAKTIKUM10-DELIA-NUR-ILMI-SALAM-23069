<x-app-layout>
    ...
    <div class="container p-4 mx-auto">
        <div class="overflow-x-auto">
            ...
            <div class="mb-4 rounded-lgbg-red-50 p-4 text-red-560">
                {{ session('error')}}
            </div>
            @endif
        ...
        <thead>
            <tr class="bg-gray-100">
                ...
                <!-- Sorting Product Name -->
                 <th class="px-4 py-2 text-gray-600 border border-gray-200">
                    <a href="{{ route('product-index', 
                    ['sort' => 'product_name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" 
                ...
                <!-- Sorting Unit -->
                 <th class="px-4 py-2 text-gray-600 border border-gray-200">
                    <a href="{{ route('product-index', 
                    ['sort' => 'unit', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" 
                    class="flex justify-between items-center">
                ...
                <!-- Sorting Type -->
                 <th class="px-4 py-2 text-gray-600 border border-gray-200">
                    <a href="{{ route('product-index', 
                    ['sort' => 'type', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" 
                    class="flex justify-between items-center">
            ...
            <th class="px-4 py-2 text-gray-600 border border-gray-200">
                <a href="{{ route('product-index', 
                ['sort' => 'information', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" 
                class="flex justify-between items-center">
            ...
            <th class="px-4 py-2 text-gray-600 border border-gray-200">
            <a href="{{ route('product-index',
             ['sort' => 'qty', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" 
             class="flex justify-between items-center">
        ...
        <th class="px-4 py-2 text-gray-600 border border-gray-200">
            <a href="{{ route('product-index', 
            ['sort' => 'producer', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" 
            class="flex justify-between items-center">
        ...
        <tbody>
            ...
            <tr class="bg-white">
                <td class="px-4 py-2 border border-gray-200 px-4 py-2">{{$item->id}}</td>
                <td class="px-4 py-2 border border-gray-200 px-4 py-2 hover:text-blue-500 hover:underline">
                                ...
                                <!-- pagination -->
                                 <div class="mt-4">
                                    {{ $data->appends(['search' => request('search'), 
                                        'sort' => request('sort'), 'direction' => request('direction')])->links() }}
                                 </div>
                            </div>
                        </div>
